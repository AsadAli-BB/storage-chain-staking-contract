...
contract UpdatedV3DynamicStakingContract is Ownable {
     ...
     */
    function _addNewDurationRates(uint256 _durations, uint256 _rates) internal {
        ...
        returns (uint256)
    {
        return
            _stake2.amount.add(
                _stake2.amount.mul(_stake2.rate.mul(1e2)).div(10000)
            );
    }
    ...
    function _stakeWithChecks(
        ...
        _stake(msg.sender, durations[duration], amount);
        ...
        returns (uint256 total, uint256 totalStakeCount)
    {
        ...
        for (uint256 i = 0; i < stakes.length; i++) {
            if (!stakes[i].paid) {
                total = total.add(stakes[i].amount);
                total = total.add(
                    stakes[i].amount.mul(stakes[i].rate.mul(1e2)).div(10000)
                );
                totalStakeCount = stakes.length;
            }
        }
    ...
    function exit() external requireLiquidity {
        ...
        // safeTransfer(msg.sender, totalWithdraw);
    ...
    function unstake(uint256 _stakeId) external requireLiquidity {
        ...
        require(
            !exitStake.paid && exitStake.end < block.timestamp,
            "You cannot unstaked this now"
        );
        ...
        // rewardToken.safeTransfer(msg.sender, totalWithdraw);
        // Transfer Native tokens in form of reward
    }
        ...
        requireLiquidity
    {
        ...
        // Don't know the Actual reason of Function
    }
     ...
     */
    function addNewDurationRates(uint256 _durations, uint256 _rates)
        external
        onlyOwner
    {
    ...
    fallback() external payable {
        // Handle incoming transactions that don't match any other function
        // For example, log the transaction data and sender address
        emit FallbackTriggered(msg.data, msg.sender, msg.value);
    }
